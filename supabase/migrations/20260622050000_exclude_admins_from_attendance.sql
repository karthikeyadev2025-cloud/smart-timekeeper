-- ============================================================================
-- Exclude admin / branch-manager profiles from attendance reports.
--
-- Why: when a company signs up, their owner's profile (e.g. "RITHVIKA HOSPITAL"
-- = EMP-0001) is created in the `profiles` table with is_active=true. They
-- hold the client_admin role and never punch in. They were polluting the
-- "absent today" and "irregular staff" lists.
--
-- Fix: every "staff" query now also excludes users who have any of the
-- non-staff roles. Real staff have either the 'staff' role or no role yet.
-- ============================================================================
...
BEGIN
  ...
  WITH staff AS (
    ...
    WHERE p.tenant_id = _tenant_id
      ...
      -- Exclude tenant owners / admins / branch managers — they're not staff.
      AND NOT EXISTS (
...
-- ----------------------------------------------------------------------------
-- daily_attendance_trend — same admin-exclusion treatment.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  total AS (
    SELECT COUNT(*)::INT AS t FROM staff_ids
  ),
  per_day AS (
    ...
    WHERE ar.tenant_id = _tenant_id
      ...
      AND ar.user_id IN (SELECT id FROM staff_ids)
