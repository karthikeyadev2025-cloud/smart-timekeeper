...
import { approvePhotoChange, rejectPhotoChange } from "@/lib/photo-change.functions";
import { Check, X, Camera, Loader2 } from "lucide-react";

export const Route = createFileRoute("/_authenticated/photo-approvals")({
  component: PhotoApprovalsPage,
});

function PhotoApprovalsPage() {
  ...
  const { data: rows = [], isFetching } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase as any)
        ...
        .select("id, user_id, photo_path, status, created_at, profiles!pending_photo_changes_user_id_fkey_profiles(full_name, staff_id, avatar_url, phone, designation)")
      ...
      <div className="space-y-4">
        <header>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            Photo change approvals
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Staff who've asked to change their ID card photo. Review each one below.
          </p>
        </header>
        ...
        {isFetching ? (
          ...
          <Card className="p-10 text-center">
            ...
            <p className="text-sm font-medium">No pending photo change requests</p>
            <p className="text-xs text-muted-foreground mt-1">You'll be notified when a staff member submits one.</p>
          </Card>
        ) : (
          <div className="space-y-3">
            {rows.map((row: any) => (
              <PhotoApprovalRow key={row.id} row={row} onChanged={() => qc.invalidateQueries({ queryKey: ["photo-approvals", tenantId] })} />
            ))}
          </div>
        )}
      </div>
...
function PhotoApprovalRow({ row, onChanged }: { row: any; onChanged: () => void }) {
  ...
  useQuery({
    queryKey: ["photo-approval-urls", row.id, row.photo_path],
    queryFn: async () => {
      const [{ data: pending }, { data: current }] = await Promise.all([
        supabase.storage.from("staff-photos").createSignedUrl(row.photo_path, 300),
        supabase.storage.from("staff-photos").createSignedUrl(`${row.user_id}/profile.jpg`, 300),
      ]);
  ...
  const doApprove = async () => {
    setBusy("approve");
    try {
      await approvePhotoChange({ data: { request_id: row.id } });
      toast.success("Approved — the new photo is now live.");
      onChanged();
    } catch (e: any) {
  ...
  const doReject = async () => {
    ...
    setBusy("reject");
    try {
      await rejectPhotoChange({ data: { request_id: row.id, reason: rejectReason || undefined } });
  ...
  return (
    <Card className="p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <PhotoTile label="Current photo" url={currentUrl} initials={initials} />
        <PhotoTile label="New photo" url={pendingUrl} initials={initials} highlight />
      </div>
...
function PhotoTile({ label, url, initials, highlight }: { label: string; url: string | null; initials: string; highlight?: boolean }) {
  return (
    <div className={`rounded-lg border-2 p-2 ${highlight ? "border-primary/40 bg-primary/5" : "border-border"}`}>
      ...
      <div className="aspect-[3/4] rounded-md overflow-hidden bg-muted flex items-center justify-center">
        {url ? (
