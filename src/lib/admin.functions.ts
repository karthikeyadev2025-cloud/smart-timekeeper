...
/* ─────────────── DELETE TENANT ─────────────── */
// Wipes everything: tenant row + all RLS-cascaded data + all users in that tenant.
export const deleteTenant = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // 1. Collect every user attached to this tenant (so we can delete auth rows too)
    ...
    // 2. Delete the tenant row — ON DELETE CASCADE removes branches, shifts,
    //    attendance, payslips, leave_requests, subscriptions, payments, etc.
    ...
    // 3. Best-effort delete of auth users (don't block on individual failures —
    //    some users might be linked to other tenants)
    const errors: string[] = [];
    for (const uid of userIds) {
      // Only delete the auth user if they're not in any other tenant
    ...
    return {
      ok: true,
      users_deleted: userIds.length - errors.length,
      auth_errors: errors,
    };
  });

/* ─────────────── BROADCAST ANNOUNCEMENT ─────────────── */
// Stores a system-wide announcement in app_settings — read by the client banner.
export const broadcastAnnouncement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Upsert single 'announcement' row in app_settings (key = 'announcement')
    const { error } = await supabaseAdmin
      .from("app_settings")
      .upsert(
