...
export function downloadPayslipPdf(payslip: PayslipPdfData, ctx: PayslipContext) {
  ...
  const ptax = Number(payslip.professional_tax ?? 0);
  ...
  const absenceOnly = Number(payslip.deductions) - lateFine - pf - esi - ptax;
  ...
  if (pf > 0 || esi > 0 || ptax > 0) {
    ...
    if (ptax > 0) row("Professional tax", "- " + inr(ptax));
  }
